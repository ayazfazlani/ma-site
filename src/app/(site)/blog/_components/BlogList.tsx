          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {initialPosts.map((post, index) => (
              <motion.article
                ...
                className={`rounded-3xl overflow-hidden transition-all duration-500 group relative border shadow-sm hover:shadow-2xl ${
                  isDark
              ...
              >
                <div className="h-64 relative overflow-hidden">
                  ...
                  <div className="absolute top-6 left-6 z-10">
                    <span className={`backdrop-blur-md px-4 py-1.5 rounded-xl text-xs font-black uppercase tracking-widest leading-none shadow-sm ${
                      isDark
                ...
                <div className="p-8">
                  ...
                  <h2 className={`text-2xl font-extrabold mb-4 transition-colors line-clamp-2 leading-tight ${
                    isDark
                  ...
                  <div className={`flex items-center justify-between pt-6 border-t ${isDark ? "border-white/[0.06]" : "border-gray-100"}`}>
                    <div className="flex items-center space-x-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${
                        isDark ? "bg-primary-500/10 border border-white/5" : "bg-primary-50 border border-primary-100"
                      }`}>
                        <User className="w-5 h-5 text-primary-500" />
                      </div>
                      <span className={`text-[13px] font-bold ${isDark ? "text-neutral-300" : "text-gray-700"}`}>
                          {post.author || "Ahmed Khan"}
                      </span>
                    </div>
